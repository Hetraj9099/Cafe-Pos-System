...
const LoginGate = () => {
  ...
  const title = useMemo(() => {
    if (mode === 'signup') {
      return 'Create your cashier account';
    }
    ...
    <div className="min-h-screen bg-[linear-gradient(180deg,#fffef8_0%,#eef2ff_100%)] px-4 py-8">
      <div className="mx-auto max-w-md rounded-[32px] bg-white p-6 shadow-xl">
        ...
        <p className="mt-2 text-sm text-slate-500">
          Staff can sign in, create an account, or reset a password with an OTP.
        </p>
        <div className="mt-5 grid grid-cols-3 rounded-2xl bg-slate-100 p-1 text-sm font-medium text-slate-700">
          <button
            className={`rounded-xl px-3 py-2 ${mode === 'login' ? 'bg-white text-slate-950 shadow-sm' : ''}`}
            onClick={() => {
              setMode('login');
              setError('');
              setMessage('');
          ...
          <button
            className={`rounded-xl px-3 py-2 ${mode === 'signup' ? 'bg-white text-slate-950 shadow-sm' : ''}`}
            onClick={() => {
              setMode('signup');
              setError('');
              setMessage('');
          ...
          <button
            className={`rounded-xl px-3 py-2 ${mode === 'forgot' ? 'bg-white text-slate-950 shadow-sm' : ''}`}
            onClick={() => {
              setMode('forgot');
              setError('');
              setMessage('');
        ...
        <div className="mt-6 grid gap-3">
          ...
          {devOtp ? (
            <p className="text-sm text-amber-700">Local OTP preview: {devOtp}</p>
          ) : null}
